...
Ext.onReady(function() {
	...
	Ext.define('SurveyRecord', {
		...
		fields: [
			...
			{name: 'n_title', type: 'string'},
			{name: 'short_content', type: 'string'},
			{name: 'n_content', type: 'string'},
			{name: 'n_date', type: 'string'},
    ...
    var columns = [
         {header: '順序', dataIndex: 'n_order', sortable: true,flex : 0.5},
         {header: '發布日期', dataIndex: 'n_date', sortable: true,flex : 1},
         {header: '消息標題', dataIndex: 'n_title', sortable: true,flex : 1.5},
         {header: '消息內容', dataIndex: 'short_content', sortable: true,flex : 10}
    ];
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        tbar:[{ 
                ...
                store : Ext.create('Ext.data.Store',{ 
                    fields : ['name','value'], 
                    data : [{'name':'順序','value':'n_order'},{'name':'發布日期','value':'n_date'},
                            {'name':'消息標題', 'value': 'n_title'},{'name':'消息內容', 'value': 'n_content'}, 
    	        ...
    	        handler: function() {
    	        	...
    	        	var orderarr =[];
    	        	 store.each(function(record) {
    	        		 orderarr.push(record.get('n_id'));
    		            });
    	        	 Ext.Ajax.request({
                         url: '/controller/newslist_order.jsp',
                         params: {
       ...
       /* defaults: {
        ...
        buttons: [{
            ...
            handler: function() {
                ...
                if (form.getForm().findField("n_id").getValue() == "") {
                    // 添加
                    form.getForm().submit({
                    	timeout : 60,
                    	waitMsg : "正在處理中.....",
                        url: '/controller/newslist_save.jsp',
                        success: function(f, action) {
                            if (action.result.success == 1) {
                            	...
                            	Ext.Msg.alert('消息', action.result.msg, function() {
                                    grid.getStore().reload();
                                    form.getForm().findField("n_title").setValue("");
									Ext.getCmp('buttonSave').setText('增加');
                                });
                            }
                        },
            ...
            text: '删除',
            handler: function() {
                var id = form.getForm().findField('n_id').getValue();
                if (id == '') {
                	...
                	Ext.MessageBox.confirm('消息','確定刪除此資料?',function(btn){
                		if(btn == 'yes'){
                			Ext.Ajax.request({
                                url: '/controller/newslist_remove.jsp',
                                success: function(response) {
                                    var json = Ext.decode(response.responseText);
                                    if (json.success) {
